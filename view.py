...
def show_histogram_input(input_image):
    ...
    plt.figure("Histogram Input", figsize=(5, 3))  # ukuran lebih kecil
...
def show_histogram_output(output_image):
    ...
    plt.figure("Histogram Output", figsize=(5, 3))  # ukuran lebih kecil
...
def plot_histogram(pixels, title, ax=None):
    """Helper function untuk menampilkan histogram RGB"""
    if ax is None:
        ax = plt.gca()
    ...
    # Plot RGB dengan transparansi agar tidak terlalu padat
    ax.hist(r, bins=bins, color='red', alpha=0.5, label='Red')
    ax.hist(g, bins=bins, color='green', alpha=0.5, label='Green')
    ax.hist(b, bins=bins, color='blue', alpha=0.5, label='Blue')
